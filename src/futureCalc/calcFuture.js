...
export function calcFutureWltAndPosAndMI(posArr, wallets, orderArr, RSObj, assetD, lastTick, UPNLPrzActive, MMType, PrzLiqType, cb){
  ...
  let {pos1, UsedWltBal} = calcPos(posArr, sumMI, assetD, lastTick, UPNLPrzActive, WltBal_obj, MMType, PrzLiqType)

  calcfutureWallet(wallets, posArr, UPNLPrzActive, sumMI, UsedWltBal, assetD )
  cb && cb({posArr, wallets})
}
 ...
 */
export function calcPos(posArr, sumMI, assetD, lastTick, UPNLPrzActive, WltBal_obj, MMType, PrzLiqType){
  ...
  for(let i = 0; i < posArr.length; i++){
    ...
    if(pos.Lever == 0){
      if(MMType == 1){
        let Lever = 1/Math.max(pos.MIRMy, pos.aMIR)
  ...
  calcPos1(posArr,SumUrlForLever0,SumMM,UsedWltBal, sumMI,assetD, lastTick, WltBal_obj, SumUPNLForM, SumMMForLever0, PrzLiqType)
  
  return {posArr, UsedWltBal}
}

function calcPos1(posArr,SumUrlForLever0,SumMM,UsedWltBal, sumMI,assetD, lastTick, WltBal_obj, SumUPNLForM, SumMMForLever0, PrzLiqType){
  for(let i = 0;i< posArr.length;i++){
    let pos = posArr[i]
    let Sym = pos.Sym
