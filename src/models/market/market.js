...
module.exports = {
    ...
    createTickData: function(param){
        ...
        if (param.Sym.indexOf('CI_') > -1) {
            ...
            tick.distSym = (param.Sym).split('_')[1] + gI18n.$t('10413')//指数
    
        }else if(AssetD){
            let tick = Object.assign({}, this.tickDefault)
            tick = Object.assign(tick, param)
            let PrzMinIncSize = utils.getFloatSize(utils.getFullNum(AssetD.PrzMinInc || 0));
            let VolMinValSize = utils.getFloatSize(utils.getFullNum(AssetD.OrderMinQty || 0));
    ...
    getSymDisplayName: function(ass, Sym){
        if(ass){
            if (ass.TrdCls == 3) {
                if((ass.Flag&1) == 1){
                    return gI18n.$t('10002', {value: ass.ToC})//ass.ToC + ' 永续'
                }else{
                    return gI18n.$t('10104', {value1: ass.ToC,value2 :ass.SettleCoin})//ass.ToC + '/' + ass.SettleCoin + ' 永续'
                }
                ...
                return gI18n.$t('10255', {value1: ass.SettleCoin,value2 :new Date(ass.Expire).format('MMdd')})//ass.SettleCoin + ' 季度' + new Date(ass.Expire).format('MMdd')
            } else {
                return Sym
            }
    ...
    initHomeNeedSub: function(){
        ...
        let displaySym = window.gWsApi.displaySym
