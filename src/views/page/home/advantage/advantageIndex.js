            ...
            m('div', { class: `container introduce-item` }, [
                m('div', { class: `mt-7 w container py-8 border-1 columns ${utils.isMobile() ? 'has-bg-level-2' : ''}` }, [
                    ...
                    m('div', { class: `introduce-item column is-3 mt-7` }, [
                        ...
                        m('p', { class: `pt-5 ${utils.isMobile() ? 'ml-7 mr-7' : 'pc才有的类名xx'}` }, ["世界顶级安全团队打造、主动安全的防御系统、银行级加密、冷热钱包分层体系，保障用户资金安全！"])
                    ]),
                    // 2
                    m('div', { class: `introduce-item column is-3 mt-7` }, [
                        ...
                        m('p', { class: `pt-5 ${utils.isMobile() ? 'ml-7 mr-7' : 'pc才有的类名xx'}` }, ["华尔街金融管理团队护航，顶级风控系统、毫秒级判断、数万BTC备付金，强大的后盾实力！"])
                    ]),
                    // 3
                    m('div', { class: `introduce-item column is-3 mt-7` }, [
                        ...
                        m('p', { class: `pt-3 ${utils.isMobile() ? 'ml-7 mr-7' : 'pc才有的类名xx'}` }, ["业内领先的百万级交易撮合引擎，一站式交易服务！"])
                    ]),
                    // 4
                    m('div', { class: `introduce-item column is-3 mt-7` }, [
                        ...
                        m('p', { class: `pt-5 ${utils.isMobile() ? 'ml-7 mr-7' : 'pc才有的类名xx'}` }, ["7*24全天候专业客服团队守候，快速反馈！"])
                    ])
                ])
            ])
        ]);
