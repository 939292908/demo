...
module.exports = function (props) {
    ...
    return m('div.views-pages-home-top has-bg-sub-level-1 ', {
        ...
        m('div', { class: ` has-text-centered pt-7`, style: `background: url(${require("@/assets/img/home/vector.png").default}) no-repeat center center / 100% 100%;` }, [
            m('p', { class: `font-weight-regular pt-9 title-x-large-1 ${utils.isMobile() ? 'ml-7 mr-7' : 'pc才有的类名xx'}` }, ['最值得信任的数字资产交易平台']),
            m('p', { class: `mt-6 title-small  has-text-level-2 font-weight-regular  ${utils.isMobile() ? 'ml-8 mr-8' : 'pc才有的类名xx'}` }, ['自主研发钱包加密技术，全面保护用户数字资产安全']),
            m('button', { class: `button button-large mt-8 theme--light has-bg-primary min-width-200 ${utils.isMobile() ? ' title-small body-5 px-4 py-2' : 'title-medium  px-8 py-3'}`, onclick: this.toPage }, ['立即交易']),
            m('div', { class: `has-text-centered mt-4` }, [
                m('a', { class: `has-text-level-2 ${utils.isMobile() ? 'body-6' : 'pc才有的类名xx'}`, href: "/w/user/#!/register", target: "_blank" }, ['还没账号？立即前往账号注册 →'])
            ]),
