...
module.exports = function (props) {
    ...
    return m('div.views-pages-home-top has-bg-sub-level-1 ', {
        ...
        m('div', { class: ` has-text-centered `, style: `background: url(${require("@/assets/img/home/vector.png").default}) no-repeat center center / 100% 100%;` }, [
            ...
            m('p', { class: `mt-5 title-small font-8eight-regular has-text-level-2 font-weight-regular  ${utils.isMobile() ? 'ml-8 mr-8' : 'pc才有的类名xx'}` }, ['自主研发钱包加密技术，全面保护用户数字资产安全']),
            m('a', { class: `purchase-btn mt-8 theme--light has-bg-primary btn-2 button title-medium font-weight-regular ${utils.isMobile() ? 'button-medium font-weight-regular ' : 'pc才有的类名xx'}`, style: "", onclick: this.toPage, target: "_blank", href: "http://localhost:8080/#!/login" }, ['立即交易']),
            m('div', { class: `has-text-centered mt-4` }, [
                m('a', { class: `has-text-level-2 ${utils.isMobile() ? 'body-6' : 'pc才有的类名xx'}`, href: "http://localhost:8080/#!/register", target: "_blank" }, ['还没账号？立即前往账号注册 →'])
            ]),
