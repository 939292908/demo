...
const assetView = {
    ...
    getallMoneyVnode: function () {
        return m.fragment(m('div.data-item mb-7', [
            m('div.data-text body-4 mb-1', I18n.$t('10212') /* '资产总额' */),
            m('div.data-price title-small mb-1', `${AssetData.AssetOverview.coinToBTC}BTC`)
        ]),
        m('div.dis-flex align-center', [
            m('div.data-item', [
                m('div.data-text body-4 mb-1', I18n.$t('10213') /* '资产估值' */),
                m('div.data-price title-small mb-1', `${AssetData.AssetOverview.coinToCNY}CNY`)
            ])
        ]));
    },
    getContractVnode: function () {
        return m.fragment(m('div.data-item mb-7', [
            ...
            m('div.data-price title-small mb-1', `${AssetData.AssetOverview.coinToBTC}BTC`),
            m('div.data-text body-4', `≈ ￥${AssetData.AssetOverview.coinToCNY}CNY`)
        ]),
        m('div.dis-flex align-center', [
            m('div.data-item mr-9', [
                m('div.data-text mb-1', I18n.$t('10219') /* '保证金余额' */),
                m('div.data-price title-small mb-1', `${AssetData.AssetOverview.NLToBTC}BTC`),
                m('div.data-text body-4', `≈ ￥${AssetData.AssetOverview.NLToCRN}CNY`)
            ]),
            m('div.data-item', [
                m('div.data-text mb-1', I18n.$t('10077') /* '未实现盈亏' */),
                m('div.data-price title-small mb-1', `${AssetData.AssetOverview.UPNLToBTC}BTC`),
                m('div.data-text body-4', `≈ ￥${AssetData.AssetOverview.UPNLToCRN}CNY`)
            ])
        ]));
    },
            ...
            m('div.asset-content px-8', [
                m('.asset-tabs dis-flex align-center', [
                    AssetData.walletList.map((item, i) => m('div.tab mr-7', { onclick: this.handleActivePir.bind(this, [item, i]) }, [
                        ...
                        m('span.name body-4', { class: AssetData.walletAcId === item.activeId ? 'has-text-primary' : '' }, item.label)
                    ]))
                ]),
                m('div.asset-data-block dis-flex justify-between align-center', [
                    m('div.asset-data', [
                        ...
                        AssetData.walletAcId === 'all' ? m('div.but-list dis-flex align-center mt-5', [
                            AssetData.LBList.map(item => m('div.but-item body-4 mr-3 py-1 px-6', { onclick: () => { AssetData.handleClickLBItem(item); } }, item.label))
                        ]) : null,
                        AssetData.walletAcId === '04' ? m('div.but-list dis-flex align-center mt-5', [
                            AssetData.legalList.map(item => m('div.but-item body-4 mr-3 py-1 px-6', { onclick: () => { AssetData.handleClickLBItem(item); } }, item.label))
                        ]) : null
                    ]),
