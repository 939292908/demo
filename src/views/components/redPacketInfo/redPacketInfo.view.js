    ...
    view(vnode) {
        ...
        if (vnode.attrs.status * 1 === 0 || vnode.attrs.status * 1 === 3) {
            return m('div', { class: `has-text-left mt-3 side-px has-text-level-3 ${vnode.attrs.class || ''}` }, [
                I18n.$t('20007', { value: `${vnode.attrs.count - vnode.attrs.count2}/${vnode.attrs.count}` })/* 已领取xx个红包 */ +
                "，" +
                I18n.$t('20008'/* 共 */) +
                `${utils.toFixedForFloor(vnode.attrs.quota - vnode.attrs.quota2, 4)}/${vnode.attrs.quota}${vnode.attrs.coin}`
        ...
        if (vnode.attrs.status * 1 === 1) {
            return m('div', { class: `has-text-left mt-3 side-px has-text-level-3 ${vnode.attrs.class || ''}` }, [
                I18n.$t('20009', {
                    ...
                    value2: `${utils.toFixedForFloor(vnode.attrs.quota, 4)} ${vnode.attrs.coin}，${redPacketUtils.getEndTime(vnode.attrs.ltm - vnode.attrs.ctm)}`
                })
