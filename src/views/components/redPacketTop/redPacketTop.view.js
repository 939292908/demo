...
module.exports = {
    view(vnode) {
        return m('div', { class: `${vnode.attrs.hiddenLine ? '' : 'has-border-bottom-1'} side-px pb-3 has-line-level-4 ${vnode.attrs.class || ''}` }, [
            // 来源 guid (空 为来源自己)
            vnode.attrs.guid ? m('div', { class: `pt-5 has-text-level-1` }, [
                m('span', { class: `` }, '来自'),
                m('span', { class: `has-text-primary` }, vnode.attrs.guid),
                m('span', { class: `` }, '的')
            ]) : m('div', { class: `pt-5 has-text-level-1` }, "您发送的"),
            // 红包类型 type 0:为拼手气 / >0:普通红包
            m('div', { class: `title-medium mb-3 has-text-level-1` }, vnode.attrs.type * 1 > 0 ? "普通红包" : "拼手气红包"),
            // 留言 des
