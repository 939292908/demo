...
utils.getTransferInfo = function (p) {
    const obj = {
        1: window.gI18n.$t("10798"), // 确认中
        2: window.gI18n.$t("10134"), // 失败
        3: window.gI18n.$t("10133"), // 成功
        11: window.gI18n.$t("10141"), // 邮件确认中// 待确认(未验证)
        12: window.gI18n.$t("10146"), // 已取消(取消)
        13: window.gI18n.$t("10142"), // 待审核(已确认，待锁定资产)
        14: window.gI18n.$t("10146"), // 已取消(未确认而且已超时)
        18: window.gI18n.$t("10134"), // 失败(确认后，出金操作失败（余额不足）【结束】)
        19: window.gI18n.$t("10134"), // 失败(确认后，出金操作失败（tks宕机，网络不通）【结束】)
        24: window.gI18n.$t("10133"), // 待审核(锁定资产成功（交易核心出金成功）)
        25: window.gI18n.$t("10146"), // 已取消(用户撤销 【对冲账单，还钱】【结束】)
        26: window.gI18n.$t("10143"), // 审核中(开始后台审核，用户不可撤销（后台回调通知ucs）)
        30: window.gI18n.$t("10134"), // 失败(后台审核不通过【对冲账单，还钱】【结束】)
        35: window.gI18n.$t("10134"), // 失败(调用外部钱包打款失败 【可补发调外部钱包】)
        36: window.gI18n.$t("10145"), // 审核已通过(调用外部钱包打款成功 【把冻结资产的状态设置为不可对冲】)
        41: window.gI18n.$t("10133"), // 成功(区块链成功回调 （暂无）)
        42: window.gI18n.$t("10144"), // 处理中(区块链失败回调 （暂无）【24小时后，人工介入???】)
        50: window.gI18n.$t("10144"), // 处理中(提币失败后，已申请对冲)
        51: window.gI18n.$t("10134"), // 失败(提币失败后，对冲成功，退回资产到帐)
        52: window.gI18n.$t("10144"), // 处理中(提币失败后，对冲失败，后台可再次审批)
        101: window.gI18n.$t("10133"), // 处理中(提币失败后，对冲失败，后台可再次审批)
        102: window.gI18n.$t("10134"), // 处理中(提币失败后，对冲失败，后台可再次审批)
        110: window.gI18n.$t('11677'), // '待审核', //待审核
        111: window.gI18n.$t('11667'), // '审核失败', //审核失败
        112: window.gI18n.$t('11674'), // '已撤销', //已撤销
        113: window.gI18n.$t('12618') // '审核成功', //审核成功
    };
    return obj[p] || '';
};
utils.getWithdrawArr = function (p) {
    const obj = {
        ...
        '12': window.gI18n.$t("10146"), // 已取消(取消)
        // eslint-disable-next-line quote-props
        '13': window.gI18n.$t('10142'), // '待审核', //待审核(已确认，待锁定资产)
        // eslint-disable-next-line quote-props
        '14': window.gI18n.$t('10146'), // '已取消', //已取消(未确认而且已超时)
        // eslint-disable-next-line quote-props
        '18': window.gI18n.$t("10134"), // 失败(确认后，出金操作失败（余额不足）【结束】)
        // eslint-disable-next-line quote-props
        '19': window.gI18n.$t("10134"), // 失败(确认后，出金操作失败（tks宕机，网络不通）【结束】)
        // eslint-disable-next-line quote-props
        '24': window.gI18n.$t('10142'), // '待审核', //待审核(锁定资产成功（交易核心出金成功）)
        // eslint-disable-next-line quote-props
        '25': window.gI18n.$t("10146"), // 已取消(用户撤销 【对冲账单，还钱】【结束】)
        // eslint-disable-next-line quote-props
        '26': window.gI18n.$t("10143"), // 审核中(开始后台审核，用户不可撤销（后台回调通知ucs）)
        ...
        '35': window.gI18n.$t("10134"), // 失败(调用外部钱包打款失败 【可补发调外部钱包】)
        // eslint-disable-next-line quote-props
        '36': window.gI18n.$t('10133'), // '成功', //区块确认中(调用外部钱包打款成功 【把冻结资产的状态设置为不可对冲】)
        // eslint-disable-next-line quote-props
        '41': window.gI18n.$t("10133"), // 成功(区块链成功回调 （暂无）)
        // eslint-disable-next-line quote-props
        '42': window.gI18n.$t('10144'), // '处理中', //处理中(区块链失败回调 （暂无）【24小时后，人工介入???】)
        // eslint-disable-next-line quote-props
        '50': window.gI18n.$t('10144'), // '处理中', //处理中(提币失败后，已申请对冲)
        // eslint-disable-next-line quote-props
        '51': window.gI18n.$t("10134"), // 失败(提币失败后，对冲成功，退回资产到帐)
        // eslint-disable-next-line quote-props
        '52': window.gI18n.$t('10144') // '处理中', //处理中(提币失败后，对冲失败，后台可再次审批)
    };
    return obj[p] || '';
};
